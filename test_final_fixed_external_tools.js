// Final test script to generate a corrected external_tool_agents.py file with proper string handling
...
// Helper function to generate Python agent file content with proper string formatting
function generateFinalFixedPythonAgentFileContent(enabledTools, enabledToolGroups = null) {
  ...
  // Generate individual enabled tools (skip MCP tools)
...
try {
  console.log('=== GENERATING FINAL FIXED EXTERNAL TOOLS FILE ===');
  ...
  // Get tool groups with their tools
  ...
  console.log('Generating final fixed external tools with:');
  enabledToolGroups.forEach(group => {
  ...
  const enabledTools = [];
  const pythonFileContent = generateFinalFixedPythonAgentFileContent(enabledTools, enabledToolGroups);
  const filePath = path.join(__dirname, 'external_tool_agents.py');
  
  fs.writeFileSync(filePath, pythonFileContent);
  console.log(`\n✅ FINAL FIXED external_tool_agents.py generated successfully!`);
  ...
  try {
    ...
    const testProcess = spawn('python', ['-m', 'py_compile', 'external_tool_agents.py'], {
    ...
    testProcess.on('close', (code) => {
      if (code === 0) {
        ...
        console.log('❌ Python syntax check failed:');
        console.log(stderr);
      }
    });
  } catch (error) {
    console.log('Could not test syntax automatically, but file has been generated.');
  }
